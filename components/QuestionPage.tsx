...
interface Props {
  onComplete: (thoughtType: string, emotion: string) => void;
}
    ...
    <div className="space-y-12 text-center max-w-sm mx-auto">
      ...
      <div className="space-y-8">
        ...
        <div key={`header-${currentIdx}`} className="space-y-3 animate-stagger-slow" style={{ animationDelay: '0.1s' }}>
          <p className="text-[10px] uppercase tracking-[0.5em] text-blue-300/40 font-medium">Question {currentIdx + 1}</p>
          ...
          {currentIdx === 1 && (
            <p className="text-sm font-light text-blue-100/30 pt-1 animate-stagger-slow" style={{ animationDelay: '0.3s' }}>
              그 생각으로 인한 지금의 기분을 알아차려 볼까요?
            </p>
          )}
        </div>
      </div>

      {/* Options Grid with Slower Staggered Animation scaled for 1.5x speed */}
        ...
        {currentQuestion.options.map((option, idx) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-blue-50/70 group-hover:text-white transition-colors tracking-tight">{option.label}</span>
              <div className="w-5 h-5 rounded-full border border-white/10 flex items-center justify-center group-hover:bg-white/90 group-hover:border-white transition-all duration-700">
